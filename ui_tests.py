...
queue_header_font = tk.CTkFont(size=16,weight="bold")
...
download_icon = tk.CTkImage(dark_image=Image.open("assets/download_light.png"),light_image=Image.open("assets/download_dark.png"))
...
# QUEUE HEADER FRAME
queue_header_frame = tk.CTkFrame(queue_global_frame,height=60)
queue_header_frame.grid(row=0,column=0,sticky="ew")
queue_header_frame.grid_rowconfigure(0,weight=1)
for i in range(4):
    queue_header_frame.grid_columnconfigure(i,weight=0)
queue_header_frame.grid_columnconfigure(0,weight=3)
queue_header_frame.grid_columnconfigure(1,weight=1)
queue_header_frame.grid_columnconfigure(2,weight=1)
queue_header_frame.grid_columnconfigure(3,weight=1)

# header title
header_title = tk.CTkLabel(queue_header_frame, fg_color="transparent", width=170, height=40,font=queue_header_font, text="Title")
header_title.configure()
header_title.grid(row=0, column=0, padx=(5,5), sticky="ew")

# header length
header_length = tk.CTkLabel(queue_header_frame, fg_color="transparent", width=60, height=40, font=queue_header_font, text="Length")
header_length.grid(row=0, column=1, sticky="ew")

# header size
header_size = tk.CTkLabel(queue_header_frame, fg_color="transparent", width=60, height=40, font=queue_header_font, text="Size")
header_size.grid(row=0, column=2, sticky="ew")

# header format
header_format = tk.CTkLabel(queue_header_frame, fg_color="transparent", width=60, height=40, font=queue_header_font, text="Format")
header_format.grid(row=0, column=3, sticky="ew", padx=(0,110))


# QUEUE FRAME
queue_frame =tk.CTkScrollableFrame(queue_global_frame)
...
# QUEUE INSTANCE TEST
element_frame = tk.CTkFrame(queue_frame,height=60)
element_frame.grid(row=0, column=0, sticky="ew",pady=3)
for i in range(5):
    element_frame.grid_columnconfigure(i,weight=0)
element_frame.grid_columnconfigure(0,weight=4)
element_frame.grid_columnconfigure(1,weight=1)
element_frame.grid_columnconfigure(2,weight=1)
element_frame.grid_columnconfigure(3,weight=1)




# element title
element_title = tk.CTkTextbox(element_frame,font=element_queue_font, activate_scrollbars=True, height=80, width=180, fg_color="transparent")
...
# element length
element_length = tk.CTkTextbox(element_frame,font=element_queue_font, activate_scrollbars=False, height=35, width=80)
...
# element size
element_length = tk.CTkTextbox(element_frame,font=element_queue_font, activate_scrollbars=False, height=35, width=60)
element_length.insert("0.0","117Mo")
element_length.configure(state="disabled",wrap="none")
element_length.grid(row=0, column=2, pady=20, sticky="ew")

# element format button
element_format_button = tk.CTkSegmentedButton(element_frame,values=["MP3","MP4"],height=35,width=60)
...
# download button
download_button = tk.CTkButton(settings_frame, width=80, height=35, text="", image=download_icon)
download_button.grid(row=0, column=0 ,padx=10)

# deflault folder button
default_directory_button = tk.CTkButton(settings_frame,image=folder_icon,text="",width=80,height=35)
default_directory_button.grid(row=1 ,column=0,padx=10)

# empty queue button
empty_queue = tk.CTkButton(settings_frame,image=trash_icon,text="",width=80,height=35)
